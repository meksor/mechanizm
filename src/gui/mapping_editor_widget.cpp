...
MappingEditorWidget::MappingEditorWidget(QWidget *parent) : QWidget(parent) {
  ...
  QObject::connect(clipTable.get(), &QTableWidget::cellClicked, this,
                   &mechanizm::MappingEditorWidget::onClipCellSelected);

  sequenceTable = std::make_unique<QTableWidget>();
  sequenceTable->setColumnCount(1);
  sequenceTable->setHorizontalHeaderItem(
      0, new QTableWidgetItem("Quarter Notes Offset"));
...
void MappingEditorWidget::setMapping(mechanizm::Mapping::shared_ptr m) {
  ...
  auto t = new openshot::Timeline(
      c->Reader()->info.width, c->Reader()->info.height, c->Reader()->info.fps,
      c->Reader()->info.sample_rate, c->Reader()->info.channels,
  ...
  writer.SetVideoOptions(true, "libx264", c->Reader()->info.fps,
                         c->Reader()->info.width, c->Reader()->info.height,
                         c->Reader()->info.pixel_ratio, false, false, 300000);
